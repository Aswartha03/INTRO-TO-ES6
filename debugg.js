function createFunctionList(){
    let functions = []
    for (let i=0;i<5;i++){
        functions.push(function (){
            console.log("Index:",i)
        })
    }
    return functions
}
const functionList = createFunctionList();
// console.log(functionList)

functionList[0]();    // Expected Output: "Index: 0", Actual Output: "Index: 5"

functionList[1]();   // Expected Output: "Index: 1", Actual Output: "Index: 5"

functionList[2]();   // Expected Output: "Index: 2", Actual Output: "Index: 5"

functionList[3]();   // Expected Output: "Index: 3", Actual Output: "Index: 5"

functionList[4]();  

// Error in the given code is to that when we use var keyword insted let , it doesn't creates block scoped so for every iteration the value of i should be equal to 5 because the loop should be completes .
// solution = using let in loop insted of var